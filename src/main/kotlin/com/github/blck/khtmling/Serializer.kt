...
private fun StringBuilder.htmlingObject(obj: Any) {
    ...
    kClass.memberProperties
        .filter { !it.hasAnnotation<HtmlIgnore>() }
        .joinToStringBuilder(
            this,
            prefix = prefix,
            postfix = postfix
...
private fun KAnnotatedElement.prefixToPostfix(defaultName: String?): Pair<String, String> {
    ...
    val prefix = """
            <${tag} ${if (name != null) "title=\"${name.lowercase()}\"" else ""} ${styles?.toList().htmlingStyles()}>
        """.trimIndent()
    val postfix = "</${tag}>"
...
private fun List<HtmlStyle>?.htmlingStyles() =
    this?.joinToString(
        prefix = "style=\"",
        postfix = "\""
    ) {
...
private fun KAnnotatedElement.resolveHtmlTag(): HtmlTag =
    this.findAnnotation<HtmlProperty>()?.tag ?: HtmlTag.DIV
...
private fun KAnnotatedElement.resolveName(defaultName: String?): String? =
    this.findAnnotation<HtmlProperty>()?.name ?: defaultName
